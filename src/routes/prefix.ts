...
router.get('/search-prefix/:columnName/:searchValue', async (req: Request, res: Response) => {
    let db = req.db;
    const columnName = req.params.columnName;
    const searchValue = req.params.searchValue;

    try {
        const result = await libPrefixModel.searchPrefix(db, columnName, searchValue);
        res.send({ ok: true, statusCode: HttpStatus.OK, rows: result });
    } catch (error) {
